...
**命令历史** — 按 Up/Down 箭头键浏览之前输入的命令（最多 16 条）。重复命令自动去重。

**Tab 自动补全** — 输入命令前缀后按 Tab 键：
- 单匹配：自动补全命令名并追加空格
- 多匹配：计算公共前缀补全，并列出所有匹配的命令
- 仅补全第一个词（命令名）
...
TinyOS> help | more
```
工作原理：Shell 解析 `|` 分隔符，为每对相邻命令创建 Pipe（IPC 管道），前命令的 stdout 重定向到 pipe，后命令从 pipe 读取输入。`write <file>` 命令支持从管道读取内容（不指定文本参数时）。
